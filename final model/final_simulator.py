...
class BestPosStrategy:
    ...
    '''
    def __init__(self, df: pd.DataFrame, delay: float, sigma: float, gamma: float, k: float, A: float,  hold_time:Optional[float]) -> None:
        '''
            delay(float): delay between orders in nanoseconds
            sigma: volatility
            gamma: attitude to risk
            k: arrival rate
            A: frequency of orders in limit orderbook
        ...
        self.A = A
        self.etta = A * pow(1+gamma/k, -1-k/gamma)
        self.T_begin = 1672693200029407500 #beginning of our session
        self.T_end = 1672725599943979000   #end of our session
        self.q = []
        self.spread = []
        self.vol = []
        self.vols = [1.0]
    ...
    def get_vol(self, receive_ts):
        step = 10**4 #time interval for moving volatility
        if receive_ts in self.ts:
            index = self.df[self.df['receive_ts']==receive_ts].index[0]
            vol = self.df.loc[index-step:index,:]['ethusdt:Binance:Spot_ask_price_0'].std()
            if vol>100:
                vol = 100
    ...
    def run(self, sim: Sim ) ->\
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                self.sigma = self.get_vol(receive_ts)
                self.vol.append([self.sigma, receive_ts])
                #calculate alpha parameter (check Gueant's formula)
                self.alpha = self.k*self.gamma*self.sigma*0.5
                #calculate delta_bid and total_delta (=delta_bid+delta_ask)
                coef1, coef2 = 0.1, 0.2 #depends on asset price
                delta_b = np.log(1 + (self.gamma/self.k))/self.gamma + np.sqrt(self.alpha/self.etta)*(2*inventory*coef2+coef2)/(2*self.k)
                sum_delta = 2*np.log(1 + (self.gamma/self.k))/self.gamma + coef2*np.sqrt(self.alpha/self.etta)/self.k
                #calculate current ask/bid prices
               
                bid_price = mid - coef1*delta_b
                ask_price = bid_price + coef1*sum_delta
        ...
        return trades_list, md_list, updates_list, all_orders, self.q, self.spread, self.vol
...
def load_trades(path, nrows=10000) -> List[AnonTrade]:
    ...
    start, end = 10**5, 2*10**5
    trades = trades.loc[start:end, :] #slice trades of session
...
def load_books(path, nrows=10000) -> List[OrderbookSnapshotUpdate]:
    ...
    '''
    lobs   = pd.read_csv(path + 'lob.csv', nrows=nrows)
    start, end = 2*10**5, 5*10**5 #md slice
    lobs = lobs.loc[start:end, :] #slice lobs of session
...
def get_pnl(updates_list:List[ Union[MdUpdate, OwnTrade] ]) -> pd.DataFrame:
    ...
    btc_pos, usd_pos, pos = 0.0, 0.0, 0.0
    rebate_bid, rebate_ask = 0.99996, 1.0004
    ...
    for i, update in enumerate(updates_list):
        ...
        if isinstance(update, OwnTrade):
            ...
            if trade.side == 'BID':
                btc_pos += trade.size
                pos+=trade.size
                usd_pos -= trade.price * trade.size * rebate_bid
            elif trade.side == 'ASK':
                btc_pos -= trade.size 
                pos+=trade.size
                usd_pos += trade.price * trade.size * rebate_ask
    ...
    df = pd.DataFrame({"exchange_ts": exchange_ts, "receive_ts":receive_ts, "total":worth_arr, "BTC":posall, 
...
if __name__ == "__main__":
    PATH_TO_FILE = 'ethusdt_'
    df = pd.read_csv('ethusdt_lob.csv')
    start, end = 2*10**5, 5*10**5 #md slice
    df = df.loc[start:end,:]
    ...
    sigma, gamma, k, A = 10.0, 1.0, 1.0, 1.0 #parameters
    strategy = BestPosStrategy(df, delay, sigma, gamma, k, A, hold_time)
    trades_list, md_list, updates_list, all_orders, q, spread, vol = strategy.run(sim)
