...
class BestPosStrategy:
    ...
    def __init__(self, df: pd.DataFrame, delay: float, sigma: float, gamma: float, k: float, A: float,  hold_time:Optional[float]) -> None:
        ...
        self.T_end = 1655943884469232791   #end of our session
        self.q = []
        self.spread = []
        self.vol = []
        self.index = 0
    ...
    def run(self, sim: Sim ) ->\
        ...
        while True:
            ...
            if receive_ts - prev_time >= self.delay:
                ...
                delta_b = np.log(1 + (self.gamma/self.k))/self.gamma + np.sqrt(self.alpha/self.etta)*(2*inventory*0.2+0.2)/(2*self.k)
                sum_delta = 2*np.log(1 + (self.gamma/self.k))/self.gamma + 0.2*np.sqrt(self.alpha/self.etta)/self.k
                #calculate current ask/bid prices
                bid_price = mid - delta_b
                ask_price = bid_price + sum_delta
...
if __name__ == "__main__":
    ...
    sigma, gamma, k, A = 10.0, 0.1, 1.0, 1.0 #parameters
    strategy = BestPosStrategy(df, delay, 11.0, 0.001, 1.0, 1.0, hold_time)
    trades_list, md_list, updates_list, all_orders, q, spread, vol = strategy.run(sim)
    vol = pd.DataFrame(vol)
    vol.to_csv('moving_vol.csv')
    df = get_pnl(updates_list)
    df.to_csv('total_pnl_gueant_moving_vol.csv')
    spread = pd.DataFrame(spread)
    spread.to_csv('total_spread_gueant_moving_vol.csv')
    q = pd.DataFrame(q)
    q.to_csv('total_q_gueant_moving_vol.csv')